...
def get_reward_metrics_for_single_trial(rewards, k):
    cummulative_rewards = [np.sum(rewards[k:t]) for t in range(k,T)]
...
def get_exp_results_for_single_trial(exp_data, ground_truth):
    ...
    for agent_name, _ in exp_data.items():
        ...
        all_rewards_metrics.append(get_reward_metrics_for_single_trial(rewards, k))

    optimal_rewards = ground_truth["optimal reward"]    
    all_rewards_metrics.append(get_reward_metrics_for_single_trial(optimal_rewards, k))
...
# cumm_rewards should be of shape (NUM_TRIALS, NUM_AGENTS, NUM_TIME_STEPS)
def create_and_save_reward_fig(agent_names, cumm_rewards, k, sigma_z, sigma_r, exp_name):
    plt.figure(figsize=(8, 6))
    k = int(k)
    for i, agent in enumerate(agent_names):
        mean_value = np.mean(cumm_rewards[:,i], axis=0)
        plt.plot(range(k, T), mean_value, label=agent)
        lower_ci = mean_value + 1.0 * np.std(cumm_rewards[:,i], axis=0)
        upper_ci = mean_value - 1.0 * np.std(cumm_rewards[:,i], axis=0)
        plt.fill_between(range(k, T), lower_ci, upper_ci, alpha=0.2)
    mean_value =  np.mean(cumm_rewards[:,-1], axis=0)
    lower_ci = mean_value + 1.0 * np.std(cumm_rewards[:, -1], axis=0)
    upper_ci = mean_value - 1.0 * np.std(cumm_rewards[:, -1], axis=0)
    ...
    plt.savefig(OUTPUT_FOLDER + f"/rewards/{exp_name}_cumm_reward_plot.pdf", format='pdf', bbox_inches='tight')
    plt.close()

### Note: assumes that the "oracle" is the last idx in cumm_rewards ###
def create_and_save_regret_fig(agent_names, cumm_rewards, k, sigma_z, sigma_r, exp_name):
    plt.figure(figsize=(8, 6))
    k = int(k)
    for i, agent in enumerate(agent_names):
        regret_value = np.mean(cumm_rewards[:,-1] - cumm_rewards[:,i], axis=0)
        plt.plot(range(k, T), regret_value, label=agent)
        lower_ci = regret_value + 1.0 * np.std(cumm_rewards[:,i], axis=0)
        upper_ci = regret_value - 1.0 * np.std(cumm_rewards[:,i], axis=0)
        plt.fill_between(range(k, T), lower_ci, upper_ci, alpha=0.2)
