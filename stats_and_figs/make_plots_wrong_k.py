...
T = NUM_TIME_STEPS
EXP_ROOT_FOLDER = "../experiment_results/unknown_k/"
OUTPUT_FOLDER = "unknown_k/"
RESULTS_FILE = "/results_{}.json"
GROUND_TRUTH_FILE = "/ground_truth_{}.json"
EXP_PATTERN = re.compile(r'K_0:(\d+(\.\d+)?)')
...
def create_and_save_reward_fig(agent_names, avg_rewards, k_0, exp_name):
...
def create_and_save_regret_fig(agent_names, avg_rewards, k_0, exp_name):
    plt.figure(figsize=(8, 6))
    k = int(k_0)
    for i, agent in enumerate(agent_names):
...
def get_all_exp_results(parent_folder):
    ...
    for exp in subfolder:
        ...
        create_and_save_reward_fig(agents, exp_reward_metrics, k_0, exp)
        print(f"Making regret graph")
        create_and_save_regret_fig(agents[:-1], exp_reward_metrics[:, :-1, :], k_0, exp)
